...
print("Installing the packages if they are not already installed")

# Define the required packages
packages <- c("httr", "utils", 
              "ggplot2", "ggtext", "tidyr",
              "dplyr", "lubridate", "gghighlight",
              "sysfonts", "showtextdb", "showtext",
              "glue", "scales", "kableExtra",
              "patchwork", "forcats",
              "reshape2")
...
print("checking if fonts are downloaded in right folder, if not thet are donwloaded automatically")
if(!"Font" %in% list.dirs()){
  dir.create("Font")
}

if(!"Century Gothic.ttf" %in% list.files(path = "Font/") ){
  # File path to save the downloaded file
  file_path <- "Font/file.zip"
  
  # Download the file
  httr::GET("https://drive.google.com/uc?export=download&id=1d4p_kZPfMw2Id0yVK3oOJ_SFPYbzQe-2",
            write_disk(file_path, overwrite = TRUE))
  # Unzip the file
  unzip(file_path, exdir = "Font")

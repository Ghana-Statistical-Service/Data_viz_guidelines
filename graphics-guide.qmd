...
# Population pyramid
ggplot(df) +
  geom_bar(aes(x = age_group, y = ifelse(gender == "Male", -count, count), fill = gender), stat = "identity") +
...
ggplot(mapping=aes(x = year, y = elder)) + 
  ...
  geom_text(aes(label =scales::comma(elder)), vjust= ifelse(lollipop_df$elder<500000, -1.5,-4))+
...
df <- data.frame(
  ...
  value = c(10, 12, 4, 16, 8)
)

# Pie chart
ggplot(df) +
  geom_col(aes(x = "", y = value, fill = country)) +
  coord_polar(theta = "y")+
	gssthemes() +
...
ggplot(df, aes(x = "", y = value, fill = country)) +
  geom_col() +
	coord_polar(theta = 'y') +
	geom_text(aes(label = percent(value/sum(value, na.rm = T))),
...
ggplot(df, aes(x = "", y = value, fill = country)) +
  geom_col() +
	coord_polar(theta = 'y') +
	geom_text(aes(label = country, color = "black"), size = 8, position = position_stack(vjust = 0.5), vjust = -1.5) +
	geom_text(aes(label = percent(value/sum(value, na.rm = T))),
					 color = "white", size = 8, position = position_stack(vjust = 0.5)) +
	gssthemes() +
	theme(axis.text = element_blank(),
...
# Dumbbell chart
ggplot(df) +
  geom_segment(aes(x = customers, xend = potential_customers, y = company, yend = company), color = "gray") +
  geom_point(aes(x = customers, y = company), size = 3, color = "#3ECDB9") +
  geom_point(aes(x = potential_customers, y = company), size = 3, color = "#04BCFC")+
	gssthemes()
...
ggplot(scatter_df) +
  geom_point(aes(x = x, 
  							 y = y, 
  							 # The color argument shows different colours for points for different groups (i.e. sex)
  							 color = sex,
  							 # The shape argument shows different shapes for points for different groups (i.e. sex)
  							 shape = sex))+
	gssthemes() +
...
```{r}
Country <- c("Ghana", "Ghana", "Ghana", "Ghana", "Ghana", "Ghana", "Ghana", "Kenya", "Kenya", "Kenya", "Kenya", "Kenya", "Kenya", "Kenya")
Year <- c(1960, 1970, 1980, 1990, 2000, 2010, 2021, 1960, 1970, 1980, 1990, 2000, 2010, 2021)
Population <- c(6652288, 8801415, 11491261, 14898012, 18781516, 24965816, 31072940, 8120080, 11301394, 16417197, 23724579, 31964557, 42030676, 53771296)
population_df <- data.frame(Country, Year, Population)

ggplot(population_df, aes(x = Year, y = Population, 
													# The colour argument displays different lines (each with a unique colour) for the different groups (i.e. Country)
													color = Country)) +
	geom_line(size = 1.5) +
	geom_point(size = 3) +
	scale_x_continuous(breaks = Year, 
...
```{r}
ggplot(population_df, aes(x = Year, y = Population, color = Country)) +
	geom_line(size = 1.5) +
	geom_point(size = 3) +
	...
	geom_text(aes(label = round(Population/1000000, 2)), vjust = ifelse(Country == "Kenya", -1, 2), show.legend=FALSE) +
	scale_x_continuous(breaks = Year, 
										 limits = c(min(Year), max(Year))) +
	scale_y_continuous(labels = function(y)y/1000000,
										 limits = nicelimits,
...
population_df %>% 
	...
	mutate(country_name = ifelse(Year == max(Year), as.character(Country), NA_character_)) %>% 
	...
	geom_text(aes(label = country_name), hjust = -0.2) +
	scale_x_continuous(breaks = Year, 
										 limits = nicelimits) +
	scale_y_continuous(labels = function(y)y/1000000,
										 limits = nicelimits,
...
area_df <- data.frame(year = c(2010, 2011, 2012, 2013, 2014, 2015),
								 rural_growth = c(2.1, 2.5, 2.8, 3.2, 3.5, 3.9),
								 urban_growth = c(2.8, 3.1, 3.4, 3.7, 4.0, 4.3))
# Create the plot
ggplot(area_df, aes(x = year, y = urban_growth)) +
	geom_line(color = urban_color, size=2) +
	geom_ribbon(aes(ymin = rural_growth, ymax = urban_growth), alpha = 0.2, fill = national_color) +
	geom_line(aes(y = rural_growth), color = rural_color , size=2) +
	labs(title = "Rural and Urban Growth", x = "Year", y = "Growth (%)") +
	gssthemes()
```
...
# Sample data
parts <- c(50, 30, 15, 5)
names(parts) <- c("First", "Second", "Third", "Fourth")

# Waffle chart
waffle(parts)+gssthemes()
```
...
# Bump chart
ggplot(df, aes(x = year, y = position, color = player)) +
  geom_bump(size = 1.5) +
  geom_point(size = 6)+
	gssthemes()
...
## Heatmap
```{r}
# population_df_melted <- melt(population_df, id.vars = c("Year", "Country"))
# 
# ggplot(population_df_melted, aes(x = Year, y = Country)) +
# 	geom_tile(aes(population)) +

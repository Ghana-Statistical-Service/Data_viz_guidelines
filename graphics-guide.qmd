...
This guide sets out a distinct style for reports from the Ghana Statistical Service (GSS). A consistent style of publishing reports is necessary to deliver this mandate. It will also support the institutional vision to be a trusted provider of statistics. The code on this page shows the a variety example of plots that can be used to display statistical outputs. These plots all follow the GSS brand styleguide. For now, the plots shown here use the R `ggplot2` package to make the various plots.
...
```{r cleveland-dot-plot, fig.height = 5, eval = FALSE}
mtcars %>%
	rownames_to_column("model") %>%
	arrange(mpg) %>%
	mutate(model = factor(model, levels = .$model)) %>%
...
```{r alpha-scatter-plot, eval = FALSE}
diamonds %>%
...
```{r before-scatter-plot, eval = FALSE}
mpg %>%
...
```{r slope-plot, fig.height = 5, eval = FALSE}
...
```{r stripchart, fig.height=2, eval = FALSE}
msleep %>%
...
```{r stripchart-with-highlighting, fig.height=2, eval = FALSE}
ggplot() +
...
```{r subsetted-stripchart, fig.height=3, eval = FALSE}
library(forcats)
...
```{r kernel-density-plot, eval = FALSE}
diamonds %>%
...
```{r kernel-density-plot-filled, eval = FALSE}
diamonds %>%
...
```{r subset-geom_smooth, eval = FALSE}
...
```{r subset-geom-smooth-lm, eval = FALSE}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy, color = factor(cyl))) +
	geom_point(alpha = 0.2) +
...
```{r gghighlight-threshold, eval = FALSE}
library(gghighlight)
library(gapminder)
...
```{r gghighlight-faceting, eval = FALSE}
data %>%
...
```{r scatterplot-geom_text, eval = FALSE}
labels <- mtcars %>%
	rownames_to_column("model") %>%
...
```{r scatterplot-geom_text_repel, eval = FALSE}
library(ggrepel)
...
```{r layering-geoms-design, eval = FALSE}
table1 %>%
...
```{r layering-geoms-design-gray, eval = FALSE}
table1 %>%
...
```{r palette-subset1, eval = FALSE}
palette_urbn_main[1:4]
```

```{r palette-subset2, eval = FALSE}
